...
class LoggerFactory:
    ...
    def get_available_loggers(cls) -> Dict[str, str]:
        ...
        """
        return {
            logger_type: logger_class.__name__
            for logger_type, logger_class in cls._loggers.items()
        }

    @classmethod
    def register_logger(
        cls, logger_type: str, logger_class: Type[BaseAuditLogger]
    ) -> None:
        ...
        logger.info(
            f"Registered logger type '{logger_type}' with class {logger_class.__name__}"
        )
    ...
    def log_auto(
        ...
        **kwargs,
        ...
        try:
            ...
            if logger_type is None:
                logger.warning(
                    f"Could not determine logger type for entity: {type(entity).__name__}"
                )
                return False
        ...
        try:
            if logger_type == "entry":
                if action in ["submit", "approve", "reject", "update", "delete"]:
                    audit_logger.log_entry_action(
                        user, entity, action, request, **kwargs
                    )
                elif action in ["workflow_transition"]:
                    audit_logger.log_entry_workflow_action(
                        user, entity, action, request, **kwargs
                    )
                else:
                ...
                if "exchange_rate" in str(type(entity).__name__).lower():
                    audit_logger.log_organization_exchange_rate_action(
                        user, entity, action, request, **kwargs
                    )
                else:
                    audit_logger.log_organization_action(
                        user, entity, action, request, **kwargs
                    )
                ...
                if "team" in str(type(entity).__name__).lower():
                    ...
                    audit_logger.log_workspace_team_action(
                        user, entity, team, action, request, **kwargs
                    )
                elif "exchange_rate" in str(type(entity).__name__).lower():
                    audit_logger.log_workspace_exchange_rate_action(
                        user, entity, action, request, **kwargs
                    )
                else:
                    audit_logger.log_workspace_action(
                        user, entity, action, request, **kwargs
                    )
                ...
                if "member" in str(type(entity).__name__).lower():
                    ...
                    audit_logger.log_team_member_action(
                        user, team, member, action, request, **kwargs
                    )
                else:
                    audit_logger.log_team_action(
                        user, entity, action, request, **kwargs
                    )
                ...
                if action.startswith("permission_"):
                    ...
                    audit_logger.log_file_operation(
                        user, entity, action.replace("file_", ""), request, **kwargs
                    )
                elif action == "operation_failure":
                    operation = kwargs.get("operation", "")
                    error_details = kwargs.get("error_details", {})
                    audit_logger.log_operation_failure(
                        user, operation, error_details, request, **kwargs
                    )
                else:
