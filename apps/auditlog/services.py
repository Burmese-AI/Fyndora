    ...
    *, user, action_type, target_entity=None, workspace=None, metadata=None
    ...
    try:
        ...
        if workspace is None and target_entity:
            ...
            # Priority 6: Target entity is related to organization, get organization's first workspace
                ...
                if hasattr(target_entity.organization, "workspaces"):
                    org_workspace = target_entity.organization.workspaces.filter(status='active').first()
                    if org_workspace:
        ...
        if workspace is None and user and hasattr(user, 'organization_memberships'):
            ...
            if active_membership and hasattr(active_membership.organization, 'workspaces'):
                user_workspace = active_membership.organization.workspaces.filter(status='active').first()
                if user_workspace:
        ...
        elif target_entity and hasattr(target_entity, 'workspace') and target_entity.workspace:
...
def audit_create_authentication_event(*, user, action_type, workspace=None, metadata=None):
    """
