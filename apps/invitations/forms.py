...
class InvitationCreateForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'email': forms.EmailInput(attrs={'class': 'input input-bordered w-full', 'placeholder': 'Enter invitee email'}),
            'expired_at': forms.DateInput(attrs={'type': 'datetime-local', 'class': 'input input-bordered w-full'}),
        }
    ...
    def clean(self):
        ...
        if not OrganizationMember.objects.filter(user=self.user, organization=self.organization).exists():
            raise forms.ValidationError("You must be a member of the organization to send an invitation.")
        
        # Check if email belongs to someone existing in the provied organization
        User = get_user_model()
        user = User.objects.filter(email=email).first()
        if user and OrganizationMember.objects.filter(organization=self.organization, user=user).exists():
            raise forms.ValidationError("User with this email is already a member of the organization.")     
