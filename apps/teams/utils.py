        ...
        if workspace_team_group is not None:
            try:
                workspace_team_group.user_set.add(
                    new_team_member.organization_member.user
                )
            except Exception as e:
                print(f"Error in adding user to workspace_team_group: {str(e)}")


def remove_user_from_workspace_team_group(joined_workspace_teams, team_member):
    for workspace_team in joined_workspace_teams:
        workspace_team_group_name = (
            f"Workspace Team - {workspace_team.workspace_team_id}"
        )
        ...
        if workspace_team_group is not None:
            try:
                workspace_team_group.user_set.remove(
                    team_member.organization_member.user
                )
            except Exception as e:
                print(f"Error in removing user from workspace_team_group: {str(e)}")
