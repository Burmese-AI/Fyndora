...
class TestSoftDeleteQuerySet(TestCase):
    ...
    def test_delete_soft_delete(self):
        ...
        self.assertEqual(
            SoftDeleteTestModel.all_objects.count(), original_count
        )  # Still exists in DB
        self.assertIsNotNone(
            SoftDeleteTestModel.all_objects.get(id=test_model.id).deleted_at
        )
    ...
    def test_hard_delete(self):
        ...
        self.assertEqual(
            result, (1, {"core.SoftDeleteTestModel": 1})
        )  # Django delete returns (count, {model: count})
        self.assertEqual(
            SoftDeleteTestModel.all_objects.count(), original_count - 1
        )  # Actually removed from DB

        # Verify it's completely gone
        self.assertFalse(
            SoftDeleteTestModel.all_objects.filter(id=test_model.id).exists()
        )
    ...
    def test_delete_multiple_objects(self):
        ...
        self.assertFalse(
            SoftDeleteTestModel.objects.filter(
                id__in=[test_model4.id, test_model5.id]
            ).exists()
        )
    ...
    def test_hard_delete_multiple_objects(self):
        ...
        self.assertFalse(
            SoftDeleteTestModel.all_objects.filter(
                id__in=[test_model4.id, test_model5.id]
            ).exists()
        )
    ...
    def test_empty_queryset_operations(self):
        ...
        empty_queryset = SoftDeleteQuerySet(
            SoftDeleteTestModel, using="default"
