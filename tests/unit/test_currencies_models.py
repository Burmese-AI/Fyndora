...
class TestCurrencyModel(TestCase):
    ...
    def test_currency_hard_delete(self):
        ...
        self.assertFalse(Currency.all_objects.filter(currency_id=currency_id).exists())
...
class TestExchangeRateBaseModel(TestCase):
    ...
    def test_exchange_rate_base_model_related_names(self):
        ...
        self.assertEqual(
            currency_field.remote_field.related_name, "%(app_label)s_%(class)s_related"
        )
        self.assertEqual(
            currency_field.remote_field.related_query_name, "%(app_label)s_%(class)s"
        )
        self.assertEqual(
            added_by_field.remote_field.related_name,
            "%(app_label)s_added_%(class)s_set",
        )
