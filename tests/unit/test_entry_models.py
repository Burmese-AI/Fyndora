...
class TestEntryModel:
    ...
    def test_entry_with_minimal_required_fields(self):
        ...
        workspace_team = WorkspaceTeamFactory(workspace=workspace, team=team_member.team)
...
class TestEntryValidation:
    ...
    @pytest.mark.parametrize("invalid_amount,error_expected", [
        (Decimal("-10.00"), True),  # Negative amounts not allowed
        (Decimal("0.00"), True),    # Zero amount not allowed (min 0.01)
        (Decimal("99999999.99"), False),  # Max value that fits in precision 10, scale 2
...
class TestEntryRelationships:
    ...
    @pytest.mark.skip(reason="Requires attachments table which may not be available in test environment")
...
class TestEntryFactories:
    ...
    def test_entry_factory_creates_valid_entry(self):
        ...
        assert entry.entry_id is not None
        assert entry.submitted_by_team_member is not None or entry.submitted_by_org_member is not None
        assert entry.organization is not None
    ...
    @pytest.mark.parametrize("factory_class,expected_status,has_reviewer", [
    ...
    def test_status_specific_factories(self, factory_class, expected_status, has_reviewer):
    ...
    @pytest.mark.parametrize("factory_class,expected_type", [
