...
class TestInvitationCreateForm(TestCase):
    ...
    def test_form_valid_data(self):
        ...
        form_data = {"email": "test@example.com", "expired_at": future_date}
    ...
    def test_form_invalid_email(self):
        ...
        future_date = timezone.now() + timedelta(days=7)
        form_data = {"email": "invalid-email", "expired_at": future_date}
    ...
    def test_form_past_expiration_date(self):
        ...
        self.assertIn(
            "Expiration date must be in the future", str(form.errors["expired_at"])
        )
    ...
    def test_form_inviter_not_organization_member(self):
        ...
        form = InvitationCreateForm(
            data=form_data, organization=self.organization, user=non_member
        )

        self.assertFalse(form.is_valid())
        self.assertIn("__all__", form.errors)
        self.assertIn(
            "You must be a member of the organization", str(form.errors["__all__"])
        )
    ...
    def test_form_invitee_already_member(self):
        ...
        OrganizationMemberFactory(
            organization=self.organization, user=existing_member_user
        )

        future_date = timezone.now() + timedelta(days=7)
        form_data = {"email": "existing@example.com", "expired_at": future_date}
    ...
    def test_form_missing_email(self):
        ...
        future_date = timezone.now() + timedelta(days=7)
        form_data = {"expired_at": future_date}
    ...
    def test_form_initialization_without_organization(self):
        ...
        form = InvitationCreateForm(data=form_data, user=self.user)
    ...
    def test_form_initialization_without_invited_by(self):
        ...
        # This should work but validation will fail
        form = InvitationCreateForm(data=form_data, organization=self.organization)
    ...
    def test_form_clean_method_order(self):
        ...
        past_date = timezone.now() - timedelta(days=1)
        form_data = {"email": "existing@example.com", "expired_at": past_date}
