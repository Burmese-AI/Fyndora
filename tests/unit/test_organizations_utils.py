...
class TestRemovePermissionsFromMember(TestCase):
    ...
    def test_remove_permissions_from_member_with_workspace_admin_role(
        self,
        mock_revoke_team_member,
        mock_revoke_coordinator,
        mock_revoke_reviewer,
        mock_revoke_admin,
    ):
    ...
    @patch("apps.organizations.utils.revoke_team_coordinator_permission")
    @patch("apps.organizations.utils.revoke_workspace_team_member_permission")
    def test_remove_permissions_from_member_with_operations_reviewer_role(
        self,
        mock_revoke_team_member,
        mock_revoke_coordinator,
        mock_revoke_reviewer,
    ...
    @patch("apps.organizations.utils.revoke_team_coordinator_permission")
    @patch("apps.organizations.utils.revoke_workspace_team_member_permission")
    def test_remove_permissions_from_member_with_team_coordinator_role(
        self,
        mock_revoke_team_member,
        mock_revoke_coordinator,
        mock_revoke_reviewer,
    ...
    @patch("apps.organizations.utils.revoke_team_coordinator_permission")
    @patch("apps.organizations.utils.revoke_workspace_team_member_permission")
    def test_remove_permissions_from_member_with_team_memberships(
        self,
        mock_revoke_team_member,
        mock_revoke_coordinator,
        mock_revoke_reviewer,
        ...
        workspace_team = WorkspaceTeam.objects.create(workspace=workspace, team=team)

        remove_permissions_from_member(self.member, self.organization)

        # Verify workspace team member permission was revoked
        mock_revoke_team_member.assert_called_once_with(
            self.member.user, workspace_team
        )

        # Verify team membership was deleted
        self.assertFalse(
            TeamMemberFactory._meta.model.objects.filter(pk=team_membership.pk).exists()
        )
    ...
    @patch("apps.organizations.utils.revoke_team_coordinator_permission")
    @patch("apps.organizations.utils.revoke_workspace_team_member_permission")
    def test_remove_permissions_from_member_with_multiple_roles(
        self,
        mock_revoke_team_member,
        mock_revoke_coordinator,
        mock_revoke_reviewer,
    ...
    @patch("apps.organizations.utils.revoke_team_coordinator_permission")
    @patch("apps.organizations.utils.revoke_workspace_team_member_permission")
    def test_remove_permissions_from_member_with_no_roles(
        self,
        mock_revoke_team_member,
        mock_revoke_coordinator,
        mock_revoke_reviewer,
...
class TestExtractOrganizationExchangeRateContext(TestCase):
    ...
    def test_extract_organization_exchange_rate_context_with_rate(self):
        ...
        self.assertEqual(
            context["organization_title"], exchange_rate.organization.title
        )
