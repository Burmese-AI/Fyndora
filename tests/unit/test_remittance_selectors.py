...
class TestGetRemittancesUnderOrganization:
    ...
    def test_get_remittances_by_organization_and_workspace(self):
        ...
        for remittance in result:
            assert remittance.workspace_team.workspace.workspace_id == self.workspace1.workspace_id
    ...
    def test_get_remittances_combined_filters(self):
        ...
        found_target = any(r.remittance_id == target_remittance.remittance_id for r in result)
        assert found_target
...
class TestRemittanceSelectorsEdgeCases:
    ...
    def test_search_with_special_characters(self):
        ...
        organization = OrganizationFactory()
        workspace = WorkspaceFactory(organization=organization, title="Workspace-Alpha & Beta")
        ...
        found_match = any(
            "Alpha & Beta" in r.workspace_team.workspace.title or 
            "Alpha & Beta" in r.workspace_team.team.title 
            for r in result
        )
        assert found_match
